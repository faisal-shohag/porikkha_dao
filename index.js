...
var localphone = localStorage.getItem('phone'),
    ...
    db.ref('hscUsers/'+localtoken).on('value', snap => {
      //console.log(snap.val());
      $('.avatar').html(`<a class="modal-trigger" href="#user-info-modal"><img src="${snap.val().avatar}" /></a>`);
      $('.username').html(`${snap.val().username}`);
      $('.modal-avatar').html(`<img src="${snap.val().avatar}" />`);
      $('.college').text(snap.val().college);
      $('.district').html(`<span class="white-text district">From <b>${snap.val().district}</b></span>
      `)
    ...
    exams.on('value', snap => {
      ...
      snap.forEach(item => {
       ...
       // console.log(results.length);
        var html = `
        <a class="modal-trigger"   href="#eachExam"><div class="exam-card" id="${item.key}">
        <div class="exam-title">${item.val()[0].title}</div>
        <div class="exam-details"> প্রশ্ন: ${item.val()[0].nq} টি | সময়: ${item.val()[0].time} মিনিট | মাইনাস মার্কস: ${item.val()[0].forWrong} | <div style="" class="exam-users"> ${results.length} জন</div></div>
        </div></a>
        `
...
var examLength = 0;
    $(document).on('click', '.exam-card', function(){
     ...
     var correctAns=[]
      ...
      eachExamRef.once('value', snap => {
        ...
        $('.title').html(`${snap.val()[0].title} <div class="exam-timer"> <div id="timer">--:--</div> <small class="red-text warn"></small></div
  ...
  $(document).on('click', '.ld-open', function(){
    $('.ld-content').html('')
  const ldRef = db.ref('exams/'+examToken+'/0/results').orderByChild('correct');
  var ld_count = 1;
  var ldData = [];
    ...
    var ss1 = ''; 
      function startTimer(duration, display) {
        ...
        interval = setInterval(function () {
            ...
            ss += numbers[parseInt(seconds%10)];
            if(mn === "০০")display.html(`<span style="color: var(--danger)">মাত্র ${ss} সেকেন্ড বাকি</span>`);
  ...
  jQuery(function ($) {
      ...
      $("input[type='radio']").on('click', function(){
          ...
          if(markedCount === examLength-1){
            $('.markedCount').html(`<div style="color: var(--success); font-size: 20px; transition: 1s;">${markedCount}/${examLength-1}</div>`);
...
//Exam Engine End
  ...
  $('#sub').unbind().click(function() {
    ...
    M.toast({html: 'If you get 0, we give you one more chance!', classes: 'rounded green'});
  }
