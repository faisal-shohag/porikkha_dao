...
$(document).ready(function(){
    ...
    login.addEventListener('submit', e => {
        ...
        if(!found){
            ...
            M.toast({html: 'এই ফোন নম্বরটি রেজিস্ট্রেশন করা হয়নি!', classes: 'rounded red'});
        }else{
        ...
        e.preventDefault();
           ...
           var username = register.username.value,
            ...
            users.once('value', snap => {
               ...
               if(found){
                   M.toast({html: 'এই ফোন নম্বরটি দিয়ে রেজিস্ট্রেশন করা হয়েছে!', classes: 'rounded red'});
               }else{
                if(phone.length != 11){
                    M.toast({html: 'তোমার মোবাইল নম্বর ১১ অংকের হতে হবে!', classes: 'rounded red'});
                   }else if(avatar === undefined){
                       M.toast({html: 'একটি প্রোফাইল ফটো আপলোড করো!', classes: 'rounded red'}); 
                   }else{
                    // uploading photo
                    $('.loading').show();
                    var metadata ={
                      ...
                      var uploadTask = storage.ref().child('avatar/' + username).put(avatar, metadata);
                      uploadTask.on(firebase.storage.TaskEvent.STATE_CHANGED, 
                          function(snapshot){
                              var progress = (snapshot.bytesTransferred / snapshot.totalBytes)
                              M.toast({html: 'আপলোড হচ্ছে... ' + progress + '%', classes: 'rounded orange toast'});
                              ...
                              switch(error.code) {
                                  ...
                                  case 'storage/unknown':
                                      console.log('Unknown Error Occured,  inspect error.serverResponse');
                                      break;
                              }
                          }, function() {
                              uploadTask.snapshot.ref.getDownloadURL().then(function(downloadURL) {
                                  M.toast({html: 'আপলোড হয়েছে!', classes: 'rounded green toast'});
                               ...
                               users.push(registrationData);
                                $('.loading').hide()
                                   ...
                                   <center><a class="modal-trigger btn purple lgbtn" href="#login"> এখন লগইন করো!</a></center>
                                   `)
                              });
                          });
                    // uploading photo end
